...
/**
 * Guards the offline Bible data: every per-chapter verse count in versification.json (which drives
 * the dropdowns) must match the actual number of verses in the bundled KJV text (kjv.json). If
 * either asset drifts, the dropdowns would offer verses that don't exist offline (or hide ones that
 * do), so this test fails the build instead.
 *
...
class KjvVersificationIntegrityTest {
    ...
    fun `versification matches bundled KJV exactly`() {
        ...
        val versification =
            json.decodeFromString(VersificationFile.serializer(), versFile.readText())
