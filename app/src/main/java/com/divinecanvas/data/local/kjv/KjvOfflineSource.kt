...
/**
 * Serves the complete, public-domain KJV bundled in assets so KJV works fully offline. The 4.5 MB
 * dataset is parsed once, lazily, and held in memory.
 */
@Singleton
class KjvOfflineSource
@Inject
constructor(
    ...
    suspend fun getText(bookOrder: Int, chapter: Int, verse: Int): String? =
        withContext(ioDispatcher) {
            ...
            val raw =
                loaded
                    .getOrNull(bookOrder - 1)
                    ?.chapters
                    ?.getOrNull(chapter - 1)
                    ?.getOrNull(verse - 1) ?: return@withContext null
            clean(raw)
        }
    ...
    private suspend fun ensureLoaded(): List<KjvBook> {
        ...
        return mutex.withLock { books ?: parse().also { books = it } }
    }
